...
> ⚠️ `mock.module()` mutates a process-global registry with no restore API. If any test file uses it, run tests via a script that includes `--isolate` (e.g. `"test": "bun test --isolate"`) or mocks leak across files. See [mocking.md](references/mocking.md).
