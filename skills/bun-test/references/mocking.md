...
### `mock.module()` — Process-Global Registry Pitfall

`mock.module(specifier, factory)` replaces a module for the **entire test process**, not just the current file. Bun's default runner shares one global and one module registry across all test files, so a mock registered in one file leaks into every later-loading file that imports the same resolved module. There is **no restore or unmock API** (`mock.restore()` does not undo module mocks; upstream issues oven-sh/bun#7823 and #6040 are open).

**Rule: projects that use `mock.module()` must run tests with per-file isolation.** Set it in the package.json script so it is impossible to forget:
...
- `--isolate`: fresh global + module registry per test file. Required when any file uses `mock.module()`.
- `--parallel` implies `--isolate`; `--no-isolate` opts out.
- No `bunfig.toml [test]` key exists for isolation — only CLI flags work.

Symptom of the leak: a test file fails with `SyntaxError: Export named 'X' not found in module '...config.ts'` even though that file never mocks it — an earlier file replaced the whole module with a partial mock.

**Narrow the blast radius** when files may share a run: import the real module before mocking and spread it, so unmocked exports stay real:

```typescript
// ❌ Replaces the whole module — other exports vanish process-wide
mock.module('./config.ts', () => ({
    isProviderEnabled: (name: string) => enabled[name] ?? true,
}));

// ✅ Preserves every export you don't override
const real = await import('./config.ts');
mock.module('./config.ts', () => ({
    ...real,
    isProviderEnabled: (name: string) => enabled[name] ?? true,
}));
...
Remember: `mock.module()` executes in order, not hoisted — static imports after it won't see the mock. Use dynamic `await import()` after mock setup.
