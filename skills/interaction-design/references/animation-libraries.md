...
The successor to Framer Motion — production-grade animation library with native browser performance and JavaScript flexibility. Install via `npm install motion`.

Import from `"motion/react"` for React components and hooks. Easing utilities import from `"motion"`.
...
function InteractiveCard() {
  return (
    <motion.div
      ...
      className="p-6 bg-background rounded-lg shadow-xs text-foreground"
    >
      Hover or tap me
    </motion.div>
  );
    ...
    <Reorder.Group axis="y" values={items} onReorder={setItems}>
      {items.map((item) => (
        <Reorder.Item
          key={item.id}
          value={item}
          className="bg-background p-4 rounded-lg shadow-xs mb-2
