...
The `project`, `branchName`, `description`, and `userStories` fields must be at the **ROOT** level. Do NOT wrap them in a `"prd"` object.
...
Keep the structure flat. Only the fields defined in the Output Format are valid.
...
Each story has a `dependsOn` array listing the IDs of stories it requires to be completed first.

- **First story**: `"dependsOn": []`
- **Subsequent stories**: List only **direct** dependencies, not transitive ones (e.g., if US-003 depends on US-002 which depends on US-001, US-003 only lists `["US-002"]`).
- **No circular dependencies** allowed. If A depends on B, B cannot depend on A.
...
6. **dependsOn**: Set correctly for each story based on its direct dependencies
...
1. **Match Stories**: Pair stories from the PRD to stories in `prd.json` (by **Title**).
2. **Handle Updates**:
   - **New Story**: Append to `userStories` with the next available `US-xxx` ID.
   - **Existing Story**:
...
3. **Re-calculate Dependencies**: Ensure `dependsOn` is correct after merge.
...
- [ ] **Schema is flat** (no wrapper objects, correct field names, uses `passes` not `status`)
